...
// @desc    Generate dynamic test with RANDOM questions
...
exports.generateTest = async (req, res, next) => {
  try {
    ...
    if (totalAvailable < questionCount) {
      return res.status(400).json({
        success: false,
        message: `Only ${totalAvailable} questions available. Please select fewer questions.`
      });
    }
...
// @desc    Submit test with detailed logging
...
exports.submitTest = async (req, res, next) => {
  try {
    ...
    const processedAnswers = answers.map(answer => {
      ...
      if (!question) {
        console.warn(`⚠️ Question ${answer.question} not found`);
        return null;
      }
      ...
      if (!isDynamicTest) {
        ...
        question.save().catch(err => console.error('Error updating question stats:', err));
      }
    ...
    console.log(`📊 Results: ${correctAnswers}/${test.totalQuestions} (${percentage}%)`);
    ...
    console.log(`✅ Test attempt saved: ${testAttempt.id}`);
